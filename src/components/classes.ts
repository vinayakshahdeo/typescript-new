  ...
  */
  ...
  class ElectricCar extends Car {
    ...
    public getElectricCarInfo() {
      ...
      console.log(
        `${this.make} ${this.model} with battery capacity of ${this.batteryCapacity} kWh`
      );
    }
  }
  ...
  class Admin extends Users {
    // protected is used to make a property of a parent class available for use in child classes
  ...
  class Personns {
    ...
    public get name(): string {
      //a getter doesn't require a corresponding setter
  ...
  /* STATIC */
  class MathUtils {
    static PI = 3.14159; // static property belongs to the class itself not to instances
  ...
  class Config {
    ...
    static {
      //invoked once when the class is loaded into memory
  ...
  class Repository<T extends Identifiable> {
    //union of T with Identifiable type
    private items: T[] = [];
  ...
  type Userr = Identifiable & {
    //extending Identifiable type	with additional properties
    name: string;
    email: string;
  };
  ...
  function timeStampMixin<T extends Constructor>(Base: T) {
    //function only accepts a constuctor or a class type
  ...
  class Employyee {
    static companyname = 'Zerodha'; //static property
    constructor(
      public name: string,
      public age: number,
