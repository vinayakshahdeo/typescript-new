namespace GenericsDemo {
  ...
  const myParam3 = function <U>(param: U): U {
    //generic function expression example
    return param;
  };
  ...
  const stringAndNumberPair: KeyValuePair<string, number> = {
    key: 'age',
    value: 30,
  };

  const arrayNumberPair: KeyValuePair<number[], number> = {
    key: [1, 2, 3],
    value: 100,
  };
  ...
  const idOfEvents: UnionOfKeysOfEvents = 'id';
  ...
  const dateOfEvents: UnionOfKeysOfEvents = 'date';
