namespace EnumDemo {
  ...
  const myObject: { [x: string]: number } = {};
  myObject['key'] = 42;
  ...
  const myInterfaceArray: ReadonlyArray<MyInterface> = [{ name: 'John' }, { name: 'Jane' }];
  // console.log(myInterfaceArray);

  type MyComputedEnum = 1 | 2 | 'three' | (() => number);
  ...
  const numberArray: number[] = [4, 5, 6];
  const stringArray: Array<string> = ['a', 'b', 'c'];
  const booleanArray: boolean[] = [true, false, true];
  const mixedArray: Array<number | string | boolean> = [1, 'two', 3, true, 'four'];
  const mixedTypeArray: (number | string | boolean)[] = [5, 'six', false, 7, 'eight'];
  ...
  const tupleExample: [number, string, boolean] = [1, 'Hello', true];
  ...
  const tupleExample2: [string, number, string?] = ['Age', 30]; // added an optional element to the end
  ...
  const readonlyArray2: readonly number[] = [6, 7, 8, 9, 10];
  // number.push(6); // Error: Property 'push' does not exist on type 'readonly number[]'.
  // /*  Read Only Tuple  */
  const readOnlyTuple: readonly [string, number] = ['Age', 25];
  ...
  const personInfo: ReadOnlyTupleType = ['John Doe', 'male', 28];
  ...
  const anotherName = myName.toUpperCase();
  ...
  const greeting: ReadOnlyString = 'Hello, World!';
  ...
  const anotherDirection: Direction3 = Direction3.East;
  ...
  const enum Edirection {
    //not compiled to JS code as an object to be used for very large enums to reduce runtime overhead
    Up,
    Down,
    Left,
    Right,
  }
  const eDirection: Edirection = Edirection.Left;
